...
* Each **<u>Agent</u>** focuses on a specific skill or responsibility and contributes its part of the work.

* A shared **<u>State</u>** lets all agents access and update the same context, allowing them to build on each other’s progress.

* The **<u>Router</u>** decides which agent should act next, using either round-robin or intelligent LLM-based routing.
...
A Pool can be thought of as a controller that organizes multiple agents, maintains a shared **<u>State</u>** for them to collaborate through, and uses a **<u>Router</u>** to decide which agent should act next.

<div className='fd-steps [&_h3]:fd-step'>

### User Input Added to State
The user’s message is written into the shared **<u>State</u>** so every agent can access it.

### Router Selects the Next Agent
The **<u>Router</u>** determines which agent should act next.
...
By default, pools use round-robin routing, where **<u>Agents</u>** take turns in order.
You can also plug in a custom router to make more intelligent decisions based on the task.

For all routing options, including round-robin, LLM-based routing, and custom router functions, see the **<u>Routers</u>** .
...
| `router` | `RouterFn \| RoutingAgent` | Custom router function or routing agent | No |
| `max_iter` | `int` | Maximum agent executions (default: `5`) | No |

\* If model is not specified for agents, the `default_model` is used.
